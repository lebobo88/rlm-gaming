...
4. **Delegate code → engineering** — persist the slice artifacts, emit a `PRD` + scoped `DEV_TASK`s to `engineering` with their `context_refs`, and repeat engine/perf/authority constraints in each task’s `instructions`. Set `pp_profile: game-dev-<engine>` and `pp_team: game-feature-team` (or `game-netcode-team` for the online layer).
5. **Delegate assets → garland (CREATIVE layer)** — The Artisan emits a schema-valid `CREATIVE_BRIEF` then concept `ASSET_JOB`s. **The Sculptor** persists the DCC contract and emits 3D `ASSET_JOB`s (`model_type: mesh|rig`, `context_refs`, `output_bucket`) to garland's `blender-model`/`blender-rig`, holding `mesh-topology-budget`; The Conductor emits audio `ASSET_JOB`s; The Choreographer specs the rig/animation and holds `rig-quality`; all generative jobs set `provenance_required: true` so `governance-c2pa` signs them. Fires `ai-content-provenance` on any gen-AI asset.
