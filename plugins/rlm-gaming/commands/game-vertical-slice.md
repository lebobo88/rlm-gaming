...
5. **Delegate assets → garland (CREATIVE layer)** — The Artisan emits `CREATIVE_BRIEF` + concept `ASSET_JOB`s; **The Sculptor** writes the DCC contract and emits 3D `ASSET_JOB`s (`model_type: mesh|rig`) to garland's `blender-model`/`blender-rig` (existing blender-mcp), holding `mesh-topology-budget`; The Conductor emits audio `ASSET_JOB`s; The Choreographer specs the rig/animation and holds `rig-quality`; all `provenance_required: true` so `governance-c2pa` signs them. Fires `ai-content-provenance` on any gen-AI asset.
