...
4. **Delegate code → engineering** — persist each design artifact, emit a `PRD` with its `context_refs`, then scoped `DEV_TASK`s whose `instructions` repeat the engine, perf, and authority constraints. Set `pp_profile: game-dev-<engine>` and `pp_team: <picked team>`. Engineering runs triage → profile → taxonomy → stage loop and returns a `DECISION_RECORD`.
